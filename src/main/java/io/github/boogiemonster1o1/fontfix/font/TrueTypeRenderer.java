...
import io.github.boogiemonster1o1.fontfix.font.text.TextAlign;
...
public class TrueTypeRenderer implements TextRendererProvider {
     ...
     * Config values
     */
    public static boolean sGlobalRenderer = true;
    ...
    public float drawFromCanvas(@Nullable String str, float x, float y, int r, int g, int b, int a, TextAlign align) {
        ...
        TextRenderNode node = this.processor.lookupVanillaNode(str, Style.EMPTY);
     ...
     */
    @Deprecated
    float drawStringGlobal(@Nullable String str, float startX, float startY, int red, int green, int blue, int alpha, boolean isShadow, Matrix4f matrix, VertexConsumerProvider typeBuffer, int packedLight) {
        /* Check for invalid arguments */
        if (str == null || str.isEmpty()) {
            return 0;
        }
        ...
        if (typeBuffer instanceof VertexConsumerProvider.Immediate) {
            ((VertexConsumerProvider.Immediate) typeBuffer).draw();
        }
    ...
    public float getStringWidth(@Nullable String str) {
        ...
        /* Return total horizontal advance (slightly wider than the bounding box, but close enough for centering strings) */
        return this.processor.lookupVanillaNode(str, Style.EMPTY).advance;
    }
    ...
    private int sizeString(@Nullable String str, float width, final boolean breakAtSpaces) {
        ...
        TextCacheProcessor.Glyph[] glyphs = this.processor.getOrCacheString(str).glyphs;
        ...
        while (index < glyphs.length && advance <= width) {
            ...
            if (breakAtSpaces) {
                char c = str.charAt(glyphs[index].getStringIndex());
                if (c == ' ') {
                    wsIndex = index;
                } else if (c == '\n') {
                    wsIndex = index;
            ...
            float nextAdvance = advance + glyphs[index].getAdvance();
        ...
        return index < glyphs.length ? glyphs[index].getStringIndex() : str.length();
    }
    ...
    public int sizeStringToWidth(String str, float width) {
        return this.sizeString(str, width, false);
    }
