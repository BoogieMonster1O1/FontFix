...
import io.github.boogiemonster1o1.fontfix.Color3i;
...
import io.github.boogiemonster1o1.fontfix.font.node.DigitGlyphInfo;
import io.github.boogiemonster1o1.fontfix.font.node.GlyphRenderInfo;
import io.github.boogiemonster1o1.fontfix.font.node.RandomGlyphInfo;
import io.github.boogiemonster1o1.fontfix.font.node.StandardGlyphInfo;
...
public class TextCacheProcessor {
     ...
     * Config values
     */
    public static int sDefaultFontSize = 16;
     ...
     */
    @NotNull
    public TextRenderNode lookupVanillaNode(@NotNull CharSequence string, @NotNull Style style) {
     ...
     */
    @Nullable
    public static Formatting fromFormattingCode(char code) {
        int i = "0123456789abcdefklmnor".indexOf(Character.toLowerCase(code));
        return i != -1 ? Formatting.values()[i] : null;
    }

    @NotNull
    private TextRenderNode generateVanillaNode(VanillaTextKey key, @NotNull CharSequence string, @NotNull final Style style) {
        // Async work
        if (!RenderSystem.isOnRenderThread()) {
            // The game thread is equal to render thread now
            synchronized (this.lock) {
                MinecraftClient.getInstance()
                        .submit(() -> this.generateVanillaNode(key, string, style))
                        .whenComplete((n, t) -> {
                            this.atomicNode.set(n);
                            synchronized (this.lock) {
                                this.lock.notify();
                            }
                        });
                ...
                TextRenderNode node = this.atomicNode.get();
    ...
    @NotNull
    @Deprecated
    public Entry getOrCacheString(@NotNull String str) {
        /*
        ...
        entry = new Entry();
        int length = this.extractFormattingCodes(entry, str, text); // return total string length except formatting codes
     ...
     */
    @NotNull
    private char[] resolveFormattingCodes(@NotNull TextProcessData data, @NotNull final CharSequence string, @NotNull Style style) {
        int shift = 0;
        ...
        for (int next = 0; next < limit; next++) {
            if (string.charAt(next) == '\u00a7') {
                Formatting formatting = fromFormattingCode(string.charAt(next + 1));
    ...
    /*private char[] resolveFormattingCodes(@NotNull TextProcessData data) {
     ...
     */
    private void startBidiAnalysis(TextProcessData data, @NotNull char[] text) {
        ...
        if (Bidi.requiresBidi(text, 0, text.length)) {
            ...
            if (bidi.isRightToLeft()) {
                this.layoutStyle(data, text, 0, text.length, Font.LAYOUT_RIGHT_TO_LEFT);
            }
        ...
        /* If text is entirely left-to-right, then insert an node for the entire string */
            ...
            this.layoutStyle(data, text, 0, text.length, Font.LAYOUT_LEFT_TO_RIGHT);
        }
    }
     ...
     */
    private void layoutStyle(@NotNull TextProcessData data, char[] text, int start, int limit, int flag) {
     ...
     */
    private void layoutText(TextProcessData data, char[] text, int start, int limit, int flag, @NotNull FormattingStyle style) {
        /*
        ...
        for (int next = start; next < limit; next++) {
            ...
            if (font == null) {
                ...
                if (font != f && codePoint != 32) {
                    this.layoutFont(data, text, last, next, flag, this.glyphManager.deriveFont(
        ...
        if (font != null) {
            this.layoutFont(data, text, last, limit, flag, this.glyphManager.deriveFont(
                    font, style.getFontStyle(), sDefaultFontSize), style.isObfuscated(),
                    effect);
        }
    }
    ...
    private void layoutFont(TextProcessData data, char[] text, int start, int limit, int flag, Font font, boolean random,
        ...
        if (random) {
            ...
            final TexturedGlyph[] digits = this.glyphManager.lookupDigits(font);
            final float factor = this.glyphManager.getResolutionFactor();
     ...
     * @param data data
     */
    private void adjustGlyphIndex(@NotNull TextProcessData data) {
    ...
    private void insertColorState(@NotNull TextProcessData data) {
        final List<FormattingStyle> codes = data.codes;
        final List<GlyphRenderInfo> glyphs = data.allList;
     ...
     */
    @Deprecated
    private int extractFormattingCodes(Entry cacheEntry, @NotNull String str, char[] text) {
    ...
    private float layoutBidiString(List<Glyph> glyphList, char[] text, int start, int limit, FormattingCode[] codes) {
        ...
        if (Bidi.requiresBidi(text, start, limit)) {
            ...
            /* If text is entirely right-to-left, then insert an EntryText node for the entire string */
            if (bidi.isRightToLeft()) {
                return this.layoutStyle(glyphList, text, start, limit, Font.LAYOUT_RIGHT_TO_LEFT, advance, codes);
            }
            ...
            /* Otherwise text has a mixture of LTR and RLT, and it requires full bidirectional analysis */
                ...
                for (int visualIndex = 0; visualIndex < runCount; visualIndex++) {
                    ...
                    advance = this.layoutStyle(glyphList, text, start + bidi.getRunStart(logicalIndex), start + bidi.getRunLimit(logicalIndex),
        ...
        /* If text is entirely left-to-right, then insert an EntryText node for the entire string */
            ...
            return this.layoutStyle(glyphList, text, start, limit, Font.LAYOUT_LEFT_TO_RIGHT, advance, codes);
        }
    }
