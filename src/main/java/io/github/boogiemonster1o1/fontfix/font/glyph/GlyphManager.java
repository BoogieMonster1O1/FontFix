...
public class GlyphManager {
    ...
    public static boolean sAntiAliasing = true;
    public static boolean sHighPrecision = true;
    public static boolean sEnableMipmap = false;
    public static int     sMipmapLevel = 4;
    public static int     sResolutionLevel = 2;
     ...
     */
    private final Graphics2D glyphTextureGraphics = this.glyphTextureImage.createGraphics();
     ...
     */
    private final ByteBuffer uploadBuffer = BufferUtils.createByteBuffer(this.imageData.length);
    ...
    private void loadPreferredFonts() {
        if (!sPreferredFontName.isEmpty()) {
            this.allFonts.stream().filter(f -> f.getName().equals(sPreferredFontName)).findFirst().ifPresent(this.selectedFonts::add);
        }

        try {
            Font f = Font.createFont(Font.TRUETYPE_FONT, this.getClass().getResourceAsStream("/assets/modid/biliw.otf"));
            this.selectedFonts.add(f);
        } catch (FontFormatException | IOException | NullPointerException e) {
            e.printStackTrace();
        }
        // Generally is Arial
        this.selectedFonts.add(new Font(Font.SANS_SERIF, Font.PLAIN, 72)); //size 1 > 72
    }
     ...
     */
    public GlyphVector layoutGlyphVector(@NotNull Font font, char[] text, int start, int limit, int layoutFlags) {
        return font.layoutGlyphVector(this.glyphTextureGraphics.getFontRenderContext(), text, start, limit, layoutFlags);
    }
    ...
    public Font lookupFont(int codePoint) {
        for (Font font : this.selectedFonts) {
     ...
     */
    @NotNull
    public Font deriveFont(@NotNull Font font, int fontStyle, int fontSize) {
        ...
        this.fontKeyMap.putIfAbsent(font, this.fontKeyMap.size());
        return font;
    }
    ...
    private Font lookupFont(int codePoint, int fontStyle, int fontSize) {
        ...
        /* If no supported fonts found, use the default one (first in selectedFonts) so it can draw its unknown character glyphs */
        Font font = this.selectedFonts.get(0);
    ...
    public TexturedGlyph lookupGlyph(Font font, int glyphCode) {
        ...
        long fontKey = (long) this.fontKeyMap.getInt(font) << 32;
        return this.glyphCache.computeIfAbsent(fontKey | glyphCode,
                l -> this.cacheGlyph(font, glyphCode));
    }
     ...
     */
    @NotNull
    private TexturedGlyph cacheGlyph(@NotNull Font font, int glyphCode) {

        /* There's no need to layout glyph vector, we only draw the specific glyphCode
         * which is already laid-out in TextProcessor */
        GlyphVector vector = font.createGlyphVector(this.glyphTextureGraphics.getFontRenderContext(), new int[]{glyphCode});

        Rectangle renderBounds = vector.getGlyphPixelBounds(0, this.glyphTextureGraphics.getFontRenderContext(), 0, 0);
        int renderWidth = (int) renderBounds.getWidth();
        int renderHeight = (int) renderBounds.getHeight();

        if (this.currPosX + renderWidth + GLYPH_SPACING >= TEXTURE_SIZE) {
            this.currPosX = GLYPH_SPACING;
            this.currPosY += this.currLineHeight + GLYPH_SPACING * 2;
            this.currLineHeight = 0;
        }
        if (this.currPosY + renderHeight + GLYPH_SPACING >= TEXTURE_SIZE) {
            this.currPosX = GLYPH_SPACING;
            this.currPosY = GLYPH_SPACING;
            this.allocateGlyphTexture();
        }
        ...
        this.glyphTextureGraphics.setFont(font);

        int x = this.currPosX - GLYPH_BORDER;
        int y = this.currPosY - GLYPH_BORDER;
        int width = renderWidth + GLYPH_BORDER * 2;
        int height = renderHeight + GLYPH_BORDER * 2;

        this.glyphTextureGraphics.drawGlyphVector(vector, this.currPosX - baselineX, this.currPosY - baselineY);

        this.uploadTexture(x, y, width, height);

        this.currLineHeight = Math.max(this.currLineHeight, renderHeight);
        this.currPosX += renderWidth + GLYPH_SPACING * 2;
        final float f = this.getResolutionFactor();

        return new TexturedGlyph(this.textureName, advance / f, baselineX / f, baselineY / f,
                width / f, height / f,
    ...
    public TexturedGlyph[] lookupDigits(Font font) {
        // the key should be cached in layout step
        int fontKey = this.fontKeyMap.getInt(font);
        return this.digitsMap.computeIfAbsent(fontKey,
                l -> this.cacheDigits(font));
    }
     ...
     */
    @NotNull
    private TexturedGlyph[] cacheDigits(@NotNull Font font) {
        ...
        for (int i = 0; i < 10; i++) {
            chars[0] = (char) (48 + i);
            GlyphVector vector = font.createGlyphVector(this.glyphTextureGraphics.getFontRenderContext(), chars);
            ...
            if (i == 0) {
                this.glyphTextureGraphics.drawString(String.valueOf(chars), this.currPosX - baselineX, this.currPosY - baselineY);
            } else {
                // align
                int offset = Math.round((standardRenderWidth - renderWidth) / 2.0f);
                this.glyphTextureGraphics.drawString(String.valueOf(chars), this.currPosX + offset - baselineX, this.currPosY - baselineY);
            }

            this.uploadTexture(x, y, width, height);

            this.currLineHeight = Math.max(this.currLineHeight, renderHeight);
            this.currPosX += standardRenderWidth + GLYPH_SPACING * 2;

            digits[i] = new TexturedGlyph(this.textureName,
    ...
    private void cacheGlyphs(Font font, char[] text, int start, int limit, int layoutFlags) {
        ...
        long fontKey = (long) this.fontKeyMap.get(font) << 32;
        ...
        for (int index = 0; index < numGlyphs; index++) {
            ...
            if (!vectorRendered) {
                 ...
                 */
                vectorBounds = vector.getPixelBounds(this.glyphTextureGraphics.getFontRenderContext(), 0, 0);
            ...
            if (this.currPosX + rect.width + GLYPH_SPACING > TEXTURE_WIDTH) {
                this.currPosX = GLYPH_SPACING;
                this.currPosY += this.currLineHeight + GLYPH_SPACING;
                this.currLineHeight = 0;
            }
             ...
             */
            if (this.currPosY + rect.height + GLYPH_SPACING > TEXTURE_HEIGHT) {
                ...
                this.currPosY = this.currPosX = GLYPH_SPACING;
                this.currLineHeight = 0;
            ...
            if (rect.height > this.currLineHeight) {
                this.currLineHeight = rect.height;
            }
             ...
             */
            this.glyphTextureGraphics.drawImage(this.tempStringImage,
                    this.currPosX, this.currPosY, this.currPosX + rect.width, this.currPosY + rect.height,
             ...
             */
            rect.setLocation(this.currPosX, this.currPosY);
             ...
             */
            if (dirty == null) {
                dirty = new Rectangle(this.currPosX, this.currPosY, rect.width, rect.height);
            } else {
                dirty.add(rect);
            }

            /* Advance cachePosX so the next glyph can be stored immediately to the right of this one */
            this.currPosX += rect.width + GLYPH_SPACING;
        }
    ...
    private void updateTexture(@Nullable Rectangle dirty) {
        ...
        if (dirty != null) {
            ...
            GL11.glTexSubImage2D(GL11.GL_TEXTURE_2D, 0, dirty.x, dirty.y, dirty.width, dirty.height,
                    GL11.GL_RGBA, GL11.GL_UNSIGNED_BYTE, this.uploadBuffer);
    ...
    private void uploadTexture(int x, int y, int width, int height) {
        ...
        GL11.glTexSubImage2D(GL11.GL_TEXTURE_2D, 0, x, y, width, height,
                GL11.GL_ALPHA, GL11.GL_UNSIGNED_BYTE, this.uploadBuffer);
    ...
    private void updateImageBuffer(int x, int y, int width, int height) {
        ...
        this.glyphTextureImage.getRGB(x, y, width, height, this.imageData, 0, width);
        ...
        for (int i = 0; i < width * height; i++) {
            ...
            this.uploadBuffer.put((byte) (this.imageData[i] >>> 24));
        }
    ...
    private void setRenderingHints() {
        ...
        this.glyphTextureGraphics.setRenderingHint(RenderingHints.KEY_ANTIALIASING, RenderingHints.VALUE_ANTIALIAS_OFF);
        if (sAntiAliasing) {
            this.glyphTextureGraphics.setRenderingHint(RenderingHints.KEY_TEXT_ANTIALIASING, RenderingHints.VALUE_TEXT_ANTIALIAS_ON);
        } else {
            this.glyphTextureGraphics.setRenderingHint(RenderingHints.KEY_TEXT_ANTIALIASING, RenderingHints.VALUE_TEXT_ANTIALIAS_OFF);
        }
        if (sHighPrecision) {
            this.glyphTextureGraphics.setRenderingHint(RenderingHints.KEY_FRACTIONALMETRICS, RenderingHints.VALUE_FRACTIONALMETRICS_ON);
        } else {
            this.glyphTextureGraphics.setRenderingHint(RenderingHints.KEY_FRACTIONALMETRICS, RenderingHints.VALUE_FRACTIONALMETRICS_OFF);
        }
    }
    ...
    private void allocateGlyphTexture() {
        ...
        this.glyphTextureGraphics.clearRect(0, 0, TEXTURE_SIZE, TEXTURE_SIZE);

        /* Allocate new OpenGL texture */
        this.textureGenBuffer.position(0);
        GL11.glGenTextures(this.textureGenBuffer);
        this.textureName = this.textureGenBuffer.get(0);
